...
{
	/// <summary>
	/// Obrazovka z pohladu klienta pri zobrazeni ponukanych jedal
	/// </summary>
	public class OrdersLayout : LayoutTimeBase
	{
		...
		public IEnumerable<ModifyLayoutItem> SetMeals(List<Menu> menu, int pageNum, int? selected)
		{
			...
			for (int i = 0; i < MEALS_PER_PAGE; i++)
			{
				...
				{
					yield return new ModifyLayoutItem($"menu_{i}", "text", $"{menu[index].IdMenu}: {menu[index].Items[0]?.Name}, {menu[index].Items[1]?.Name}, {menu[index].Items[2]?.Name}, {menu[index].ServingPlace?.Name}");
